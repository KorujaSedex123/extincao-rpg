...
const ARQUETIPOS_LISTA = ["O Combatente", "O Civil (Força Bruta)", "A Médica", "O Técnico", "O Líder Espiritual", "O Marginal", "A Atleta", "O Sniper", "O Mascote", "Outro / Customizado"];
...
export class BoilerplateActorSheet extends ActorSheet {
    ...
    constructor(object, options = {}) {
        ...
        else if (object.type === 'refugio') { options.width = 700; options.height = 700; }
    ...
    _prepareItems(context) {
        const armas = [], equipamentos = [], qualidades = [], defeitos = [], traumas = [];
        ...
        context.armas = armas; context.equipamentos = equipamentos;
        context.qualidades = qualidades; context.defeitos = defeitos; context.traumas = traumas;
    }
    ...
    activateListeners(html) {
        ...
        // Aqui garantimos que o clique no botão chama a função correta
        ...
        html.find('.header-archetype select').change(async (ev) => {
            ...
            if (!stats) return;
            if (await Dialog.confirm({title: "Aplicar Arquétipo?", content: "Isso vai resetar seus Atributos."})) {
    ...
    // --- FUNÇÃO DE CONTROLE DE RECURSOS (ATUALIZADA E SEGURA) ---
    async _onResourceControl(event) {
        event.preventDefault();
        event.stopPropagation(); // Impede cliques duplos/fantasmas

        const button = event.currentTarget; // Pega o botão <a> mesmo que clique no <i>
        const action = button.dataset.action; // 'plus' ou 'minus'
        const target = button.dataset.target; // 'pv' ou 'pr'
        
        // Garante que pegamos o recurso corretamente
        const resource = this.actor.system.resources[target];
        if (!resource) return;

        // Converte para NÚMERO (Essencial para não somar texto)
        ...
        // Atualiza a ficha
        await this.actor.update({ [`system.resources.${target}.value`]: value });
    }
    ...
    async _onRoll(event) {
        ...
        if (['horda-abraco', 'horda-fuga', 'usage', 'item'].includes(dataset.rollType)) return this._processRoll(dataset); 

        // Adiciona suporte a rolagem de Instinto Puro (Horda)
        ...
        new Dialog({
            title: `Rolar: ${label}`,
            content: `<form><div class="form-group"><label>Modificador (+/-)</label><input type="number" name="modifier" value="0" autofocus/></div><p style="font-size:0.8em;color:#888;">Ex: -3 (Escuridão)</p></form>`,
            buttons: {
                roll: {
                    label: "ROLAR",
    ...
    async _processRoll(dataset, modifier = 0) {
        if (dataset.rollType === 'usage') {
             ...
             if (degraded) await item.update({ "system.uso": UD_CHAIN[currentUd] || "0" });
             let message = `<div class="extincao-roll"><h3 class="roll-label">Munição: ${item.name}</h3><div class="roll-result ${degraded?'failure':'success'}">${result}</div><div class="roll-details">Dado: 1${currentUd}</div>${degraded?`<div style="color:#ff4444;font-weight:bold;">⚠️ GASTO! Caiu para: ${UD_CHAIN[currentUd]||"VAZIO"}</div>`:`<div style="color:#4eff8c;">Preservado.</div>`}</div>`;
        ...
        if (dataset.rollType === 'skill') {
            ...
            label = `${dataset.label}`;
            poolFormula = `${attrVal} + ${skillVal}`;
            if (skillVal >= 4) { successThreshold = 5; poolFormula += " [MAESTRIA]"; }
        }
        else if (dataset.key === 'instinto') { // Instinto Puro
            diceCount = 1;
            label = "INSTINTO";
            isInstinctRoll = true;
        ...
        if (modifier !== 0) { diceCount += modifier; poolFormula += ` ${modifier >= 0 ? '+' : ''}${modifier} (Mod)`; }

        if (diceCount < 1) { diceCount = 1; isInstinctRoll = true; poolFormula += " -> INSTINTO"; }
        ...
        results.forEach(val => { if (val >= successThreshold) successes++; if (val === 6) sixes++; if (val === 1) ones++; });
        ...
        if (isGlitch) { statusText = "GLITCH / CATÁSTROFE"; statusClass = "glitch"; }

        let content = `<div class="extincao-roll"><h3 class="roll-label">${label}</h3><div class="roll-result ${statusClass}">${statusText}</div>${isInstinctRoll ? '<div style="color:orange;font-size:0.8em;text-align:center;">⚠️ Dado de Instinto</div>' : ''}<div class="roll-details">${diceCount}d6 [${results.join(', ')}]<br><span style="color:#666;">Pool: ${poolFormula}</span></div>${sixes > 0 ? `<div style="margin-top:5px;border-top:1px solid #333;color:#8b0000;"><strong>CRÍTICOS: ${sixes}</strong></div>` : ''}</div>`;
